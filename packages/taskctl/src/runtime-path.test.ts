...
it("uses cwd/.data for source commands and preserves explicit empty new settings", () => {
  ...
  expect(() =>
    runtimeDataDirectory({ LARK_CODEX_DATA_DIR: "", LARK_TASKBOARD_DATA_DIR: "/old" }),
  ).toThrow("LARK_CODEX_DATA_DIR 不能为空");
});

it("maps only the moved legacy default when its old directory is absent and new runtime exists", () => {
  const home = mkdtempSync(join(tmpdir(), "lark-codex-runtime-path-"));
  const old = join(home, "Library/Application Support/Lark Codex Taskboard/data");
  const current = join(home, "Library/Application Support/Lark-Codex/data");
  ...
  try {
    ...
    expect(runtimeDataDirectory({ ...environment, LARK_CODEX_DATA_DIR: old }, home)).toBe(old);
