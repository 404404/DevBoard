---
name: manage-codexboard-legacy-archive
description: 已弃用的 v0.1.11 macOS Desktop/taskctl Skill。不得用于当前 Docker/SSH Run 部署；请通过 DevBoard Web 或飞书操作。
---

# 已弃用：manage-codexboard v0.1.11 Desktop Skill

> **停止使用本 Skill。** 它依赖 macOS `CodexBoard.app`、调用者本地 cwd、Codex Desktop 项目导入及旧 Job/Git 模型，不适用于容器化 DevBoard、远端 Workspace Mapping 或 SSH Run。当前任务、Run、审批和 Continue 必须从 DevBoard Web/飞书访问同一控制面。不要尝试把本机项目路径传给容器，也不要将 `~/.codex` 或 SSH 凭据目录挂载进容器。

当前容器不包含此 taskctl CLI 或 Skill 安装器。容器内运维（例如备份）应使用 `docker compose exec` 运行镜像内的 ops 命令，见仓库 `README.md` 和 `docs/development.md`。下方 v0.1.11 内容只为旧数据/客户端迁移留档，不构成当前使用说明。
