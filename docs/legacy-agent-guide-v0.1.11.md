# Archived v0.1.11 Agent guide (not current instructions)

[面向用户的 README](README.md)

> **当前权威说明：Docker Compose Control Plane。** 本文件后续原有 macOS Desktop/FRP 指令属于 v0.1.11 历史内容，已经 deprecated，禁止按其步骤安装、配置公网入口、导入本机 Codex 或发布 DMG。当前部署请先读 [README](README.md) 和 [Docker/反向代理指南](docs/reverse-proxy.md)。容器不包含 Agent CLI、不运行 FRP/Caddy、不读取容器 `~/.codex`；所有 Host Execution 必须经人工确认过 Host Key 的 SSH Connection。

## 当前部署的安全操作摘要

- 使用 `compose.yaml` 部署；生产必须显式设置 HTTPS `DEVBOARD_PUBLIC_ORIGIN`。
- `DEVBOARD_TRUST_PROXY` 只允许填写实际代理的 IP/CIDR；不信任任意 `X-Forwarded-*`。
- Compose 默认只向 `127.0.0.1:47823` 发布 HTTP；Admin API 的 `47824` 不发布。
- 项目目录是目标 SSH Host 视角下的绝对路径。Docker Host 也通过 SSH 连接；容器 `localhost` 不代表宿主机。
- 先扫描 Host Key，再由用户核对并确认 SHA256 指纹。Key/Agent 凭据不进入日志、镜像或对话。
- 容器升级保留 `/var/lib/devboard` 数据卷；不要将 `~/.codex`、`~/.cursor`、整个用户 home 或 Docker socket 挂进容器。
- 旧 `manage-codexboard` Skill/taskctl 已弃用，不能用来启动 SSH Run；任务和审批使用 Web/飞书。运维备份可在容器内运行 `docker compose exec -T devboard node apps/server/dist/ops.js backup`。
- 当前生产 Git/worktree 管理及任务 Git 收尾 fail-closed，尚未完成远端映射接线；不要把旧本地结果当作远端 Host 状态。

以下为历史 Desktop 指引存档，不得用于当前部署：
