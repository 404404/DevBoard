...
fn instance_lock(directory: RawFd, attempts: usize) -> Result<File, String> {
    ...
    Err("Lark-Codex 已在运行，请先正常退出旧应用后再打开；现有数据未迁移".into())
}
...
pub fn open(home: &Path, attempts: usize) -> Result<(PathBuf, File), String> {
    ...
    let legacy = identity(parent, LEGACY_NAME)?;
    if legacy.is_some() && identity(parent, NAME)?.is_some() {
        return Err("Lark-Codex 与旧版 Lark Codex Taskboard 数据目录同时存在。未合并或覆盖，请先核对两个目录后再打开应用".into());
    }
    let selected = if legacy.is_some() { LEGACY_NAME } else { NAME };
    let data = directory(parent, OsStr::new(selected), legacy.is_none())?;
    ...
    if let Some(expected) = legacy {
        if identity(parent, LEGACY_NAME)? != Some(expected) || identity(parent, NAME)?.is_some() {
            return Err("应用数据目录在迁移前发生变化，未覆盖任何目录，请重新检查".into());
        }
        let from = CString::new(LEGACY_NAME).unwrap();
    ...
    } else if identity(parent, LEGACY_NAME)?.is_some() {
...
mod tests {
    ...
    impl Fixture {
        ...
        fn legacy(&self) -> PathBuf {
            self.0.join("Library/Application Support").join(LEGACY_NAME)
        }
    }
