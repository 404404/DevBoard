# Archived v0.1.11 macOS Desktop instructions — not current deployment guidance

> 本目录仅保留历史 Tauri 源码用于迁移与审计，不属于主要产品形态、CI、容器镜像或发行流程。下方内容是旧 v0.1.11 操作文档，不能作为当前部署说明。FRP、Caddy supervisor、桌面 Codex bridge 与 DMG 更新路径已从主产品移除；请使用根目录 [Docker Compose 指南](../../docs/reverse-proxy.md)。容器不会读取旧 `frpc.toml`，也不会将容器 `~/.codex` 当作宿主机 Codex 状态。

当前支持 Apple Silicon、macOS 13 或更新版本。内置 Node 运行时。

应用根据公共访问模式启动服务：内置 FRP 启动 DevBoard 后端、Caddy 和 frpc；外部反向代理启动 DevBoard 后端和 Caddy；本机模式只启动 DevBoard 后端。关闭主窗口时隐藏窗口和 Dock 图标，仅保留菜单栏图标，服务继续运行；通过菜单栏「显示主窗口」恢复窗口和 Dock 图标。选择菜单栏「退出 DevBoard」、按 Command-Q 或使用系统应用菜单退出时，应用停止自己启动的后台进程。界面的「停止服务」仍可手动停止服务。Codex Desktop 和它已经执行的任务不会被应用关闭。
...
「使用引导」可切换查看 Web 或飞书步骤；连接配置支持内置 FRP、外部反向代理和本机三种互斥公共访问模式。飞书应用与 Web 账号可以同时使用：配置飞书凭据启用飞书入口，配置 HTTPS 并在本机「应用设置 → Web 账号」创建账号启用浏览器入口。仅使用 Web 时可同时留空两项飞书凭据。旧 `deploy/access.json` 不再控制运行方式。两项飞书凭据一起保存到 `secrets/feishu-credentials.json`，公共访问配置保存到 `config/public-access.json`，文件权限为 `0600`。内置 FRP 模式读取 `secrets/frpc.toml`；外部反向代理模式填写 Public Origin 和 Caddy 监听地址，不启动 frpc；本机模式不需要公网 Origin、Caddy 或 frpc。详见 [公共访问模式](../../docs/public-access.md) 和 [外部反向代理](../../docs/external-reverse-proxy.md)。内部桥接令牌首次启动自动生成，之后保持不变。
...
内置 FRP 模式从 `frpc.toml` 识别公网入口，并自动配置 Caddy、后端 Origin、登录 Cookie 和引导检查。外部反向代理模式以用户明确填写的 Public Origin 为准，内部 Caddy 使用 HTTP 接收代理转发，并校验 `Host` / `X-Forwarded-Proto`；本机模式固定使用回环 API 地址，不启动公网入口。Admin API 始终只监听本机。

内置 FRP 的 HTTPS 仍由 Caddy 自动签发和续期证书，需要域名解析正确且公网 443 能到达本机 Caddy。外部代理应负责公网 TLS，并把流量转发到 Caddy 监听地址；不要把 Admin 端口暴露给代理或公网。
...
产物：`apps/desktop/dist/DevBoard.app`。
...
1. 将 `DevBoard.app` 复制到「应用程序」目录；升级前先正常退出已有的 DevBoard。
2. 打开应用，按照「使用引导」选择内置 FRP、外部反向代理或本机访问模式并保存配置；使用域名时完成对应 DNS 配置。
...
codesign --verify --deep --strict "apps/desktop/dist/DevBoard.app"
```
