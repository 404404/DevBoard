...
function fixture(t) {
  ...
  const source = join(root, "skills/manage-lark-taskboard");
...
test("bundles only runnable Skill files with deterministic hashes and executable wrapper", (t) => {
  ...
  for (const file of manifest.files) {
    const destination = join(runtime, "skills/manage-lark-taskboard", file.path);
  ...
  assert.throws(() => statSync(join(runtime, "skills/manage-lark-taskboard/README.md")), {
    code: "ENOENT",
  });
});
...
test("packaging fixes file permissions even with a restrictive release umask", (t) => {
  ...
  assert.equal(
    statSync(join(runtime, "skills/manage-lark-taskboard/SKILL.md")).mode & 0o777,
    0o644,
  );
  assert.equal(
    statSync(join(runtime, "skills/manage-lark-taskboard/scripts/taskctl.sh")).mode & 0o777,
    0o755,
  );
});
