...
export async function startEmbeddedCodexBridge(config: AppConfig): Promise<EmbeddedCodexBridge> {
  ...
  return bridge.startCodexBridge({
    codexPath: config.LARK_TASKBOARD_CODEX_COMMAND,
    tokenFile: config.LARK_TASKBOARD_CODEX_TOKEN_FILE as string,
    endpoint: config.LARK_TASKBOARD_CODEX_ENDPOINT,
    projectStateFile: config.LARK_TASKBOARD_CODEX_PROJECT_STATE_FILE as string,
    projectSnapshotFile: config.LARK_TASKBOARD_CODEX_PROJECT_SNAPSHOT_FILE,
  });
}
