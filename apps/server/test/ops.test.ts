...
describe("operations CLI", () => {
  it("creates and verifies a backup with stable JSON output", async () => {
    const dataDirectory = mkdtempSync(join(tmpdir(), "lark-taskboard-ops-"));
  ...
  it("refuses an offline CLI backup while restore owns the data-directory lock", async () => {
    const dataDirectory = mkdtempSync(join(tmpdir(), "lark-taskboard-ops-locked-"));
    temporaryDirectories.push(dataDirectory);
    initializeDatabase(join(dataDirectory, "taskboard.sqlite")).close();
  ...
  it.each([47824, 80])(
    ...
    async (adminPort) => {
      const dataDirectory = mkdtempSync(join(tmpdir(), "lark-taskboard-ops-online-"));
      temporaryDirectories.push(dataDirectory);
  ...
  it.each([
    ...
    const dataDirectory = mkdtempSync(join(tmpdir(), "lark-taskboard-ops-admin-url-"));
    temporaryDirectories.push(dataDirectory);
    mkdirSync(join(dataDirectory, "run"));
    writeFileSync(
      join(dataDirectory, "run", "runtime.json"),
  ...
  it("falls back to the kernel lock when a stale runtime descriptor is unreachable", async () => {
    const dataDirectory = mkdtempSync(join(tmpdir(), "lark-taskboard-ops-stale-runtime-"));
    temporaryDirectories.push(dataDirectory);
    initializeDatabase(join(dataDirectory, "taskboard.sqlite")).close();
