...
当前为 **0.1.6 预览版**，支持 **Apple Silicon Mac · macOS 13 或更新版本**。
...
| 飞书客户端与自建应用（可选） | 使用飞书入口或 CLI 身份配对时需要；仅用 Web 看板不需要             |
...
Web 账号是可执行任务的受信任用户，不是只读访客。各账号共用看板，新建任务归当前登录用户，评论署该用户；目前不提供按项目隔离的账号权限。只给信任的人创建账号。停用或重置密码会使该账号已有浏览器登录失效。
